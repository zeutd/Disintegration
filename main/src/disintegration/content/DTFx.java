...
public class DTFx {
    ...
    lightningBlast = new Effect(40, 100, e -> {
        ...
        color(Pal2.lightningWhite);
        for(int i = 0; i < 4; i++){
            Drawf.tri(e.x, e.y, 6f, 35f * e.fout(), i*90);
        }

        color();
        for(int i = 0; i < 4; i++){
            Drawf.tri(e.x, e.y, 3f, 6f * e.fout(), i*90);
        }

        Drawf.light(e.x, e.y, circleRad * 1.6f, Pal2.lightningWhite, e.fout());
    }),
    largeLightning = new Effect(50f, 500f, e -> {
