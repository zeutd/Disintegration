...
public class DTWaves {
    public static Seq<SpawnGroup> generateOmurlo(float difficulty){
        //apply power curve to make starting sectors easier
        return generateOmurlo(Mathf.pow(difficulty, 1.12f), new Rand());
    }

    public static Seq<SpawnGroup> generateOmurlo(float difficulty, Rand rand){
        return generateOmurlo(difficulty, rand, false);
    }

    public static Seq<SpawnGroup> generateOmurlo(float difficulty, Rand rand, boolean airOnly){
        UnitType[][] species = {
                {verity, truth, solve, essence, axiom},
                {assist, strike, coverture, DTUnitTypes.attack, devastate},
                {colibri, albatross, crane, eagle, phoenix},
                {lancet, talwar, estoc, rand.chance(0.5) ? defend : spear, rand.chance(0.1) ? defend : epee}
        };
